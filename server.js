...
async function salvarLeadHubSpot(lead) {
  ...
  const body = {
    properties: {
      ...
      servico_veiculo:       lead.servico || '',
      urgencia_compra:       lead.urgencia || '',
      precisa_financiamento: lead.financiamento || '',
...
// ── ENDPOINT: BUSCA DE VEICULOS POR FILTROS ──────────────────────────────
...
// ── ENDPOINT: BUSCA DE VEICULO POR ID ───────────────────────────────────
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({
        ...
        mensagem: 'Anuncio nao encontrado no Patio Digital.',
      });
    }
